...
import { fetchWeather, getBriefWeatherInfo, PRESET_CITIES, type WeatherData } from '@/lib/weather';
...
export function WeatherTile() {
  ...
  const weatherInfo = data
    ? getBriefWeatherInfo(data.weatherCode)
    : { label: '---', icon: 'unknown' };

  const renderIcon = (iconName: string, size: string = 'w-5 h-5') => {
    switch (iconName) {
      case 'sunny':
        return <FaSun className={cn(size, 'text-yellow-500')} />;
      case 'partly-cloudy':
        return <FaCloudSun className={cn(size, 'text-yellow-500')} />;
      case 'cloudy':
        return <FaCloud className={cn(size, 'text-muted-foreground')} />;
      case 'rainy':
        return <FaCloudRain className={cn(size, 'text-blue-400')} />;
      case 'snowy':
        return <FaSnowflake className={cn(size, 'text-blue-200')} />;
      case 'thunderstorm':
        return <FaBolt className={cn(size, 'text-yellow-600')} />;
      default:
        return <FaCloudSun className={cn(size, 'text-muted-foreground opacity-50')} />;
    }
  };
    ...
    return [3, 6, 9, 12]
      .map((hoursAhead) => {
        ...
        return {
          ...
          icon: getBriefWeatherInfo(hourly.weatherCode[idx]).icon,
        };
      })
    ...
    <div className="h-full relative overflow-hidden group">
      ...
      <AnimatePresence>
        {showPicker && (
          ...
          >
            <div className="text-[10px] font-black text-muted-foreground uppercase mb-2 px-1">
              Select City
            </div>
