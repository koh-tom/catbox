import { FaFire, FaPaw } from 'react-icons/fa';
...
export function HabitTile({ streak }: HabitTileProps) {
  return (
    <CardContent className="p-6 flex items-center justify-between h-full bg-gradient-to-r from-primary/5 to-transparent">
      <div>
        <h3 className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">Weekly Streak</h3>
        <div className="flex items-center gap-1 mt-2">
          {[...Array(7)].map((_, i) => (
            <div key={i} className={cn("w-6 h-1 rounded-full", i < streak ? "bg-primary" : "bg-primary/20")} />
          ))}
        </div>
      </div>
